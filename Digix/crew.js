...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '2250576891330@s.whatsapp.net'; // ton numÃ©ro ou le groupe cible
...
â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
      *KEUTLY-MD Connected Successfully* ðŸš€
â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '2250576891330' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'KEUTLYMD');
                console.log('ðŸ“² Pairing Code:', code);
                console.log('ðŸ‘‰ Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['2250576891330@s.whatsapp.net'], // emplace par ton numÃ©ro WhatsApp 
